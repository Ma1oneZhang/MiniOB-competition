    ...
    int internal_max_size /* = -1*/, int leaf_max_size /* = -1 */)
{
  ...
  size_t tot_attr_length = 0;
  for (auto &field : fields) {
    tot_attr_length += field->len();
  }
  if (internal_max_size < 0) {
    internal_max_size = calc_internal_page_capacity(tot_attr_length);
  }
  if (leaf_max_size < 0) {
    leaf_max_size = calc_leaf_page_capacity(tot_attr_length);
  }

  char            *pdata         = header_frame->data();
  IndexFileHeader *file_header   = (IndexFileHeader *)pdata;
  file_header->tot_attr_length   = tot_attr_length;
  file_header->key_length        = tot_attr_length + sizeof(RID);
  file_header->internal_max_size = internal_max_size;
  file_header->leaf_max_size     = leaf_max_size;
  file_header->root_page         = BP_INVALID_PAGE_NUM;
  file_header->attr_count        = fields.size();
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const char *user_key, const RID &rid)
{
  ...
  memcpy(static_cast<char *>(key.get()), user_key, file_header_.tot_attr_length);
  memcpy(static_cast<char *>(key.get()) + file_header_.tot_attr_length, &rid, sizeof(rid));
  return key;
}
...
RC BplusTreeHandler::delete_entry(const char *user_key, const RID *rid)
{
  ...
  memcpy(key, user_key, file_header_.tot_attr_length);
  memcpy(key + file_header_.tot_attr_length, rid, sizeof(*rid));
    ...
    int right_len, bool right_inclusive)
{
  ...
  if (left_user_key && right_user_key) {
    const int result = tree_handler_.key_comparator_.attr_comparator(left_user_key, right_user_key);
    if (result > 0 ||  // left < right
                       // left == right but is (left,right)/[left,right) or (left,right]
        (result == 0 && (left_inclusive == false || right_inclusive == false)) || result == -1  // invalid date format
    ) {
      return RC::INVALID_ARGUMENT;
    }
  }
...
//   int32_t attr_length = tree_handler_.file_header_.tot_attr_length;
