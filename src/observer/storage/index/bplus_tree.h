...
public:
  void init(int32_t attr_count, AttrTypeInfo *attr_type_infos)
  {
    ...
    for (int i = 0; i < attr_count; i++) {
      attr_printers_[i].init(attr_type_infos[i].attr_type, attr_type_infos[i].key_length);
    }
  }
...
struct IndexFileHeader
{
  ...
  const std::string to_string()
  {
    ...
    ss << "tot_attr_length:" << tot_attr_length << ","
    ...
    for (int i = 0; i < attr_count; i++) {
      ss << i + 1 << ": type:" << attr_type_info[i].attr_type << " length:" << attr_type_info[i].key_length << ",";
    }
    ss << "];";
    return ss.str();
  }
