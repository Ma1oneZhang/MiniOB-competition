...
public:
  ...
  RC create(const char *file_name, const IndexMeta &index_meta, std::vector<const FieldMeta *> const &field_metas);
  RC open(const char *file_name, const IndexMeta &index_meta, std::vector<const FieldMeta *> const &field_metas);
  RC close();
  ...
  RC drop(const char *base_dir, const char *table_name) override;
